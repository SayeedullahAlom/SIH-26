...
Missing information must remain missing. The system must not invent values when OCR cannot find something.
...
Choose one for the first implementation. Do not put OCR code directly into `routes_inspections.py`.
...
Create `app/services/declaration_parser.py`.

**Input:** Raw OCR text
**Output:** Structured declaration
...
Use the existing `app/models/declaration.py`.
...
Make sure they agree. If changes are required:
...
Implement `POST /inspections/{inspection_id}/extract`.
...
The endpoint must verify `inspection.officer_id == current_user.id` before processing.

### Step 3.7 — Idempotency

Calling `POST /inspections/{id}/extract` multiple times must not blindly create duplicate declarations.
